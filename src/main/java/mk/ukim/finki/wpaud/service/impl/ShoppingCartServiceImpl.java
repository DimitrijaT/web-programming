...
public class ShoppingCartServiceImpl implements ShoppingCartService {
    ...
    public ShoppingCartServiceImpl(InMemoryShoppingCartRepository shoppingCartRepository,
                                   InMemoryUserRepository userRepository, ProductService productService) {
    ...
    public List<Product> listAllProductsInShoppingCart(Long cartId) {
        //.isEmpty()  or .isPresent() both work
    ...
    public ShoppingCart addProductToShoppingCart(String username, Long productId) {
        ...
        if (shoppingCart.getProducts().stream()
                .filter(i -> i.getId().equals(productId)).collect(Collectors.toList()).size() > 0)
